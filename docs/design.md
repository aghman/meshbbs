...
**Status:** Draft v0.21 — §12.6's conformance vectors; the first Phase 6 prerequisite
**Date:** 2026-08-14
...
**Phase 6 has begun at its prerequisites rather than its deliverables.** §12.6's conformance vectors are built (v0.21) — not because the freeze is close, but because the corpus is worth more before it than during it: it freezes current behaviour including its bugs, and a bug caught after the freeze is a compatibility problem rather than a fix. Two pre-freeze items remain and neither is the freeze itself. **§7.4's dictionary is still the untrained corpus** written when a post was the only thing being compressed — it predates `FILE` and `DOOR_EVENT`, both of which look nothing like prose — and training it is cheap today and an interop commitment afterwards; the conformance corpus now pins dictionary 0, so the retrained one must ship as dictionary 1 rather than replacing it. And **`N10` is still open**: the governor runs on a guessed flood multiplier of 4, every airtime figure here scales linearly with it, and the framing about to be frozen was sized against that guess. Neither blocks the other. The three items that are *not* prerequisites — the written spec, the portnum request, the FTN gateway — are all downstream of a format that has survived real radios, and §13's own caution about not rushing applies to them and not to the groundwork.
