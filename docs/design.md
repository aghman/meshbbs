...
**Status:** Draft v0.22 — the trained dictionary; two of three Phase 6 prerequisites done
**Date:** 2026-08-15
...
- **What ships is not `dict_v1`, and its corpus predates two of the record types that now use it.** The first bullet is still outstanding: there is no `zstd --train` product embedded anywhere. Dictionary 0 is a raw corpus of a couple of hundred bytes of forum vocabulary — `subject:`, `re:`, `wrote`, `posted`, common English function words — written when a post was the only thing being compressed. `FILE` (§7.5) and `DOOR_EVENT` (§9.5) both landed after it and neither looks anything like prose: a door event body is short binary fields (a game name, a nick, a `uint8`, an opaque payload) with almost no compressible text in it, and a league bundle is many of them. Nothing is broken — an unhelpful dictionary costs ratio, not correctness — but the "3–5× on a 400-byte post" figure above is a claim about forum text and no measurement backs it for either newer kind. **Train the real dictionary before the Phase 6 freeze**, on a corpus that includes bundles of all three, and gate the result with §12.7's compression benchmark. The dictionary ID is announced in the digest and old versions stay supported, so this is a cheap change today and an interop commitment after the freeze.
  ...
  - **Nothing announces dictionaries, and the second bullet above is the part still outstanding.** "Nodes announce which dictionaries they hold in their digest" was never built — the digest carries areas and nothing else (§7.3) — so a node cannot find out what a peer can read, and packing with dictionary 1 means a peer on an older build cannot read anything it sends. That is acceptable exactly once: §7.1's pre-freeze policy is that development releases make no compatibility promises, and §13 records that no two real boards have ever exchanged anything, so there is no installed base to strand. It stops being acceptable at the Phase 6 freeze, because after it a dictionary 2 could not ship at all without a way to discover who can read it. **§13 names it a freeze prerequisite.** The sneakernet path is where this bites hardest and permanently: a stick has no round trip, so there is never a negotiation to have, and a carrier is readable only by a build holding the dictionary it was packed with.
...
Two limits stated rather than implied. **The third bullet is not done and cannot be**: there is one version of each format, so there is nothing to cross. `testdata/v1/` is a directory so v2 lands beside it, and a test fails the moment a `FormatVersion` is bumped without one. And **a corpus freezes current behaviour including its bugs**, which is an argument for doing this before the freeze rather than as part of it — after the freeze a bug in the encoding stops being a bug and becomes a compatibility problem. The vectors were generated only after all eight wire-format fuzz targets ran clean, since three canonicalisation bugs in this codebase were found by fuzzing and none by reading.
...
**Phase 6 has begun at its prerequisites rather than its deliverables.** §12.6's conformance vectors are built (v0.21) and §7.4's dictionary is trained and shipped (v0.22) — neither because the freeze is close, but because both are worth more before it than during it. A corpus freezes current behaviour including its bugs, and a bug caught after the freeze is a compatibility problem rather than a fix; a dictionary is a cheap change today and an interop commitment afterwards.

Three prerequisites remain, and none of them is the freeze itself.

**Dictionary negotiation.** §7.4 says nodes announce which dictionaries they hold in their digest. They do not: the digest carries areas and nothing else, and that half was never built. v0.22 could ship dictionary 1 as the packing default anyway only because §7.1's pre-freeze policy makes no compatibility promises and no two real boards have ever exchanged anything — a licence that expires the moment somebody deploys. **After the freeze a dictionary 2 cannot ship at all without this**, because there would be no way to find out who can read it, and §7.4's "old dictionaries stay supported" degrades into "everyone must upgrade at once", which is the thing it was written to avoid. This is the cheapest of the three and the one with the sharpest deadline.
