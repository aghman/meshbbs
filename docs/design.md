...
**Status:** Draft v0.23 — dictionary negotiation and the first cross-version corpus
**Date:** 2026-08-16
...
**The door API has its own document: [doors.md](doors.md), which specifies §9.1's contract at the wire level — including §9.5's `event.emit` and `event.poll` in its §6.7 — and records where implementation narrowed §9.1.1, §9.2, §9.4 and §11.5.**

*v0.3 added account creation and registration (§5.1, §6.7) and configuration and administration (§11).*
*v0.23 implements §7.4's second bullet — nodes announce which dictionaries they hold in their digest — which pays back the debt v0.22 took on when it shipped dictionary 1 as a flag day, and closes the last Phase 6 prerequisite that needed no radios. The mechanism is one byte and the arithmetic is the argument for putting it there: 34.4ms of channel per digest, about 8.3 seconds a day across a fifty-node mesh, some **0.2% of §1.1's federation allocation**, and nothing at all in area capacity since `MaxAreas` is a floor division and `(224-3)/10` and `(224-4)/10` are both 22. Four things it settled. **"Highest held" is complete rather than lossy**: §7.4 keeps old dictionaries supported and a set is built from 0 upward, so a bitmap would spend seven bits on gaps this design forbids. **There is no per-peer choice, because the mesh is broadcast** — one bundle is heard by everyone, so a node picks one dictionary and **one peer on an old build holds the entire mesh at its level**. That is what broadcast means rather than a defect in the rule; encoding per peer would spend the airtime of N transmissions to save the bytes of one, giving up the economy §7.2 chose fountain coding to get. So it is reported instead, naming the peer and the remedy, once per change. **Silence is not a claim**: a peer that has never sent a digest does not vote, because treating it as dictionary 0 would pin a fresh node at 0 forever, and one past `PeerTimeout` stops voting by the same definition of "gone" the digest interval already uses. And **sneakernet declares rather than negotiates, permanently**, because a stick has no round trip and never will — carrier v3 states the dictionary its bundles need so an import refuses in one sentence instead of dying inside a bundle. The larger consequence was not the feature: the digest change was the **first format bump the append-only corpus had ever faced**, and it could not be absorbed, which is §12.6's mechanism working exactly as designed. The corpus is now generations, an old one becomes the cross-version corpus, and it found a real bug on its first run — `DecodeDigest` checked the version-2 header length before the version byte, so a three-byte version-1 digest reported "truncated" and sent a sysop to look at the radios when the answer was that a peer needed upgrading. **A length rule belongs to the version that defined it**, and every gossip decoder now reads the version first. No round-trip test could have found that, because a round trip never has any old bytes.*
*v0.22 implements §7.4's first bullet — the trained dictionary this document has asked for since v0.1 and never had — and, from measuring it, corrects §7.4's headline claim. **Dictionary 1 returns 1.298× on a holdout corpus it has never seen, where plain zstd returns 1.170× and dictionary 0 returns about 1%.** That is roughly a tenth of the bytes off every bundle, and it is not the 3–5× §7.4 promises. The gap is not a shortfall, it is a category error in the original claim, and it is worth stating exactly: **3–5× is a figure about 400 bytes of post TEXT, and what gets compressed is a bundle body, which is 19–42% Ed25519 signature.** Sixty-four incompressible bytes per record put a hard ceiling of 2.4×–5.3× on real bundles depending on shape, and that ceiling assumes text and framing compress to literally zero. The honest win is at the small end, where there is no internal redundancy for a compressor to find and a dictionary supplies the model it cannot build: a lone post goes 1.10× → 1.20×, and **a lone catalog entry goes 0.96× → 1.07× — plain zstd makes it BIGGER**, and rescuing that is most of what a dictionary is carried for. A door league barely moves (1.50× → 1.53×), which is expected rather than disappointing: eight near-identical events in one record are already redundant enough to exploit unaided. Four things are worth naming. **`zstd --train` is not available and was not needed**: klauspost's `BuildDict` builds entropy tables from samples but takes dictionary content verbatim, doing none of COVER's substring selection, so the selection is eighty deterministic lines in `tools/traindict` — a build-time dependency on a C binary is how a reproducible artifact stops being reproducible (§4). **Entropy tables are most of the win**; dictionary 0 is a RAW dictionary carrying none, which is why 130 bytes of genuinely relevant vocabulary bought nothing. **The corpus is synthetic and lives in the repository**, because a zstd dictionary embeds literal fragments of its training data into the shipped binary and training on archived message bases would put third-party text of uncertain provenance inside an MIT program; train and holdout share a register and no content, and every number quoted here is from the half the trainer never saw. And **16 KiB of content was chosen by the generalisation gap, not by the ratio** — holdout is still climbing at 32 KiB, but a 33 KiB dictionary built from a 113 KiB corpus is carrying a third of its own training data, and at 64 KiB it triples on data it has seen while gaining 0.026 on data it has not. Two things are recorded rather than skated over: **nothing negotiates dictionaries**, because §7.4's "nodes announce which dictionaries they hold in their digest" was never built and the digest carries only areas — so packing with dictionary 1 is a flag day, which is free exactly once and only now (§7.1 pre-freeze, and §13 records that no two real boards have ever exchanged anything) and which §13 now names a hard freeze prerequisite; and **§12.7's compression gate did not exist** in any form, nor did any other benchmark in the tree, so the ratios above are now assertions that fail rather than figures in a document that age.*
*v0.21 implements §12.6 — the frozen conformance corpus — which §12.6 itself said the Phase 6 freeze needed "well before then", and which is the first work in this document aimed squarely at Phase 6 rather than at Phase 5. The gap it closes is worth naming precisely, because "the encoders are well tested" was true and beside the point: **every existing test of an encoder is a round trip**, and a round trip proves the two halves agree with each other rather than which bytes they agreed on. Flipping the record timestamp to little-endian in the encoder AND the decoder together was measured against the whole suite — every package passes, including `record`'s own — and only the corpus fails. That is the entire justification for it existing. Four things it settled. **The corpus is self-describing**: each vector carries its input beside its expected output and the test rebuilds the output from the input, because an artifact a third-party implementation is supposed to test against cannot assume a Go builder, and §12.6's fourth bullet says a third party is the point. **Immutability is mechanical, not a convention** — the generator recomputes every existing vector, refuses to write when one would change, and refuses to let one be removed — because the obvious fix for a red build is to regenerate, and that is precisely the wrong one. **Bundles pin the UNCOMPRESSED framing**, since `Pack`'s output moves with any zstd upgrade and is supposed to move again when §7.4's real dictionary is trained; the compressed direction is covered by frozen blobs asserted only to decode, which incidentally is what now stops dictionary 0 being edited in place instead of superseded. And the **fountain mask** is the highest-value entry, not the record envelope: the mask never travels on the wire, so an implementation deriving it differently produces symbols that decode to garbage silently, with every length and signature agreeing — there is nowhere else in BSMP where being wrong is this quiet. Three exports were added to reach it (`bundle.EncodeBody`/`DecodeBody`, `bundle.Dictionary0`, `fountain.Mask`) and dictionary 0's corpus moved out of the CLI wiring into the bundle package, where a wire-format constant belongs. Two limits are recorded rather than skated over: **cross-version vectors do not exist**, because there is only one version of each format and §12.6's third bullet has nothing to compare yet — `testdata/v1/` is a directory so that v2 lands beside it and a test fails until it does; and **freezing bytes freezes current behaviour including its bugs**, which is why the corpus was generated only after all eight wire-format fuzz targets ran clean, three canonicalisation bugs in this codebase having been found by fuzzing rather than by reading.*
*v0.20 implements §6.5's fetch path 2 — sneakernet carriers, and the request queue that gives them something to answer — which completes Phase 5. A carrier is a container of the existing bundle format plus the sender's version vectors, rather than a second record encoding to freeze at Phase 6, and carrying the vectors is what makes a hand-off work at all: there is no round trip in one, so a stick has to say both "here is what I have" and "here is what I know" for the reply to be computable without a conversation. The queue is the half this document had been waiting on since v0.17 removed "queued for next exchange" from the file browser for being a promise nothing would keep — that sentence is back, with the one qualification §6.5's own text left out and a person at a terminal needs: **there is no date**, because an exchange happens when somebody carries a drive somewhere. Three decisions are worth naming. **A request names a content hash and not a file**, which is not a compromise but all the requester has ever seen — a `FILE` record carries the truncated hash — and is the stronger ask, since it is answered by the bytes or not at all. **A request bounds the answer**: the reply leg carries what was asked for and nothing else, which is the difference a queue buys over the blunt "everything you do not have" that an opening carrier is stuck with. And **an arrival is filed without being announced**, because the network already knows about the file from the holder's own record and a second origin restating it would spend §1.1's budget saying nothing. Two things are recorded rather than skated over: no two real boards have exchanged a stick, only two instances through the built binary; and a carried file nobody requested is content with no catalog entry to reach it by, which the import says plainly rather than leaving as an orphan the sysop discovers later.*
*v0.19 implements §8.2 tier 3 — `meshbbs-key`, the helper that keeps a user's DM private key on their own machine — which the roadmap calls the one Phase 5 item that is not optional. It is an addition rather than a rewrite, exactly as §8.2 said it would be if the Phase 1 boundary held: it did, and the whole server-side mechanism turns out to be one row, a public key with the wrapped column NULL. Nothing in sending, discovery, verification or delivery needed touching, because none of them ever reached for the private half. NULLing the wrapped column is the load-bearing part rather than tidiness — a stale wrapped key would be found by OpenDM, unwrapped with the session passphrase, and used to hand back plaintext for a user who had been told their key was theirs alone. Two things are recorded honestly rather than skated over: `user dm-key` is a sysop command and that is a **weak** control, not a strong one; and tier 3 costs the user a copy, a paste and a passphrase per message, which is why tier 2 stays the default. §8.2.1 has the shape as built.*

*v0.18 implements §9.5 — `DOOR_EVENT`, the record type that carries game events between boards — and, because §9.5 asked door traffic for "its own sub-budget like an area", §6.3's per-area `airtime_share`, which had been in §11.5 since Phase 1 and in no Go file. A league **is** an area, of kind `door`, sharing the `areas` table with forums and file areas for the same reason those two share it: a parallel table would mean reimplementing anti-entropy, digests, bundles and the CLI, and independently named leagues would collide with forums in the one global `AreaTag` namespace. Batching had to move **inside** the record, and the arithmetic is the whole design: framing is 89 bytes of which 64 is the signature, a typical event is 23, so six events cost 672 bytes as separate records and 197 in one — a bundle hoists origin and area but every record inside it keeps its own signature. Emitting is **not** a fifth API level, because levels nest and a fifth above `act_as_user` would claim that reporting a game result is more authority than posting as the user; it is a second axis on level 3, the mirror image of the announce area, requiring a federated area where announce requires a local one. Two things this document said are corrected in place rather than paraphrased: §9.5's caveat put door events "at the bottom of the priority order with forum posts" and they are now **below the file catalog**, because a stale catalog entry is still true and a stale battle report is noise; and §6.3's per-area sub-budget stops being a specification. **No league has ever run between two real boards** — what exists is the local emit/queue/batch/publish path and a crossing that runs end to end in the simulator — and `N10` is still open, which is why the batch window is derived from what the area's share buys rather than configured at a value chosen for a guessed R.*
*v0.17 implements §6.5 and §7.5 — file areas, a content-addressed blob store, and `FILE` records that replicate a catalog while the files never move — which completes Phase 3's scope in code. Scoping it found that the local half had never been built: the file areas were a raw SFTP directory with no database behind them, so the thing to replicate did not exist. File areas now share the `areas` table with a `kind` column, because an `AreaTag` is a global namespace and two independently named tables would let a message area and a file area collide into one tag. Two places where implementation contradicted this document are corrected in place rather than quietly paraphrased: the "queued for next exchange" wording in §6.5 and §7.5 **is not shipped**, because the sneakernet queue is a Phase 5 deliverable and printing a promise nothing will keep is worse than the spinner those bullets warn against; and §7.5's rule that the mesh link rejects file payloads is **enforced one layer lower than it says**, at the record codec, since a link sees opaque datagrams and a check there could only be the size heuristic the rule forbids. The largest possible catalog entry is 211 bytes against an 8 KiB record body limit, so there is nowhere to put content and no threshold to relax. A fuzz target found the `FILE` body had a second wire form on its first run (an overlong varint), fixed with the re-encode-and-compare defence the envelope already used.*
...
- **Per-area airtime sub-budget.** Given ~10 originated packets/day/node at 50 instances (at the guessed R of 4, §14), one chatty area can starve every other. Each federated area gets a share of the node's governor allocation. This is also the mechanism that makes an FTN-bridged echo safe (§7.7).
  - **Specified in Phase 1, implemented in Phase 5.** `airtime_share` appeared in §11.5 and in no Go file until a door league needed it (§9.5), so the general mechanism was built and the league became its first user rather than its reason. Four things it settled that this bullet did not say. A share is a **rate** with its own bucket, which is what distinguishes it from the class reserve of §7.6 — a reserve stops a class spending the last of the budget and does nothing about how fast an area spends the first of it. The shares of **federated** areas must sum to no more than 1, enforced as a refusal where the column is written rather than as a warning, because an instance that has promised away 130% of its budget has made a decision it cannot keep and would discover it weeks later as areas mysteriously starving each other; local-only areas do not count, since they spend no mesh airtime. And an area's bucket is **floored at one full packet** however small its share, because below that the area is not slowed but silently switched off — which is also why the sum rule is checked against the configured fractions rather than claiming a precision the floor does not permit. Shares are re-read on the federation tick, not at startup: a cap that needs a restart is a cap nobody reaches for while the mesh is busy.
...
- Be honest in the UI: a file with no IP-reachable holder shows "available by request only — queued for next exchange," with the requesting user notified when it lands. Not an error, not a spinner that never resolves.
  - **Built in Phase 5, and the wording is back.** v0.17 shipped this screen deliberately WITHOUT that sentence, because the queue did not exist and printing it would have been the exact dishonesty this bullet is about. It exists now: `r` in the file browser writes a row, the row rides every sneakernet carrier as a content hash, a board holding those bytes answers with them, and the arrival is announced to the person who asked the next time they log in.
  - **A request names a hash, not a file.** The requester has never held the content — all it has seen is the `FILE` record, which carries the truncated hash of §6.5 rather than all 32 bytes — so the truncation is not a choice the queue makes, it is the whole of what the requester knows. It is also the stronger way to ask: "send me `utils/kermit.zip`" is answered by whatever the holder filed under that name, while "send me the bytes hashing to this" is answered by the bytes or not at all, and the receiver re-hashes on the way in. The name is kept locally, because it is where the arrival gets filed and what the person will look for.
  - **There is no date, and the UI does not invent one.** A hand-off happens when somebody carries a stick somewhere. "Queued" is the whole of the promise, the same shape as a door event's answer ([doors.md](doors.md) §6.7) — recorded, and going out when the medium allows. Anything more specific would be the false promise this bullet warns about wearing a better suit.
  - **A request also bounds the answer.** With a queue to work from, the reply leg carries what was asked for AND NOTHING ELSE. That is the difference the queue buys over the blunt "everything you do not have" an opening carrier is stuck with: a stick has a size and the trip has a cost, and a board that asked for one file should not receive forty because they happened to fit.
  - **An arrival is not re-announced.** The file is filed with no `FILE` record of its own. The network already knows about it — the entry being answered came from the holder's own record — and a second origin restating the same content would spend §1.1's shared airtime saying nothing new.
...
- If the passphrase is shared with the login password, changing the login password re-wraps the DM key — which works only while the plaintext key is in memory, i.e. during an authenticated session. A sysop-forced password reset therefore **cannot** re-wrap it, so a sysop reset must either preserve the DM passphrase separately or explicitly warn that DM history will be lost. Get this right in Phase 1; it is exactly the kind of thing that becomes unfixable once real users have real mail.
  - **None of this applies to a tier-3 user** (§8.2.1). Their wrapped key is NULL, so a password change has nothing to re-wrap and a sysop reset destroys nothing — the key it would have discarded is on their own machine. That is the same property from the other side: the server cannot lose what it never held.
...
1. **Digests never carry full version vectors.** A digest carries, per federated area, `{area_tag(4) | rolling_hash(4) | count(2)}` = 10 bytes, behind a four-byte header of `{type | version | dictionary | area_count}`. Ten areas = 104 bytes, one packet. Full vectors are exchanged **unicast, on demand**, only when a rolling-hash mismatch proves divergence.
   - **The dictionary byte is §7.4's announcement**, added in v0.23 and the reason gossip is at wire version 2. It costs one byte per digest — about 8.3 seconds of channel a day across a fifty-node mesh, some 0.2% of §1.1's federation allocation — and nothing at all in area capacity, since `MaxAreas` is a floor division and both `(224-3)/10` and `(224-4)/10` are 22.
...
- **What ships is not `dict_v1`, and its corpus predates two of the record types that now use it.** The first bullet is still outstanding: there is no `zstd --train` product embedded anywhere. Dictionary 0 is a raw corpus of a couple of hundred bytes of forum vocabulary — `subject:`, `re:`, `wrote`, `posted`, common English function words — written when a post was the only thing being compressed. `FILE` (§7.5) and `DOOR_EVENT` (§9.5) both landed after it and neither looks anything like prose: a door event body is short binary fields (a game name, a nick, a `uint8`, an opaque payload) with almost no compressible text in it, and a league bundle is many of them. Nothing is broken — an unhelpful dictionary costs ratio, not correctness — but the "3–5× on a 400-byte post" figure above is a claim about forum text and no measurement backs it for either newer kind. **Train the real dictionary before the Phase 6 freeze**, on a corpus that includes bundles of all three, and gate the result with §12.7's compression benchmark. The dictionary ID is announced in the digest and old versions stay supported, so this is a cheap change today and an interop commitment after the freeze.
  - **Ship it as dictionary 1; do not edit dictionary 0.** As of v0.21 the corpus content lives in `bundle.Dictionary0Corpus` rather than in the code that wires up a server, because it is a wire-format constant and not a configuration choice, and §12.6 pins a bundle packed against it. Rewriting it in place would leave every peer on an older build unable to decode anything sent by a newer one, under a dictionary ID that claims they agree — which is the precise failure "old versions stay supported" exists to prevent. The conformance corpus is what makes that attempt a red build rather than a field report.
  - **Done in v0.22, and the 3–5× above is wrong.** `internal/bundle/dict1.zdict`, trained by `tools/traindict` from `internal/dictcorpus`, is what nodes now compress with; dictionary 0 stays readable forever and is pinned by §12.6. Measured on a holdout corpus the trainer never saw: **1.298× against plain zstd's 1.170×**, or about a tenth of the bytes off everything that goes on the air. The rest of this bullet is what measuring taught, and it corrects the section above rather than paraphrasing it.
    - **3–5× was never reachable, because it is a claim about the wrong thing.** It describes 400 bytes of post text. What is compressed is a bundle body, and a bundle body is 19–42% signature — 64 incompressible bytes per record. The ceiling on a five-post bundle is 4.3× and on a six-entry catalog bundle 2.4×, and those assume text and framing compress to *nothing*. Any future claim about compression here should be stated against a bundle, not against a payload.
    - **The value is at the small end**, which is also where the budget bites hardest. A lone post: 1.10× → 1.20×. A lone catalog entry: 0.96× → 1.07×, where the 0.96 means plain zstd *expands* it — there is not enough data for a compressor to build a model, which is exactly the condition a dictionary exists to fix. A door league goes 1.50× → 1.53× and that is fine: eight near-identical events in one record are already redundant enough to exploit without help.
    - **Entropy tables, not vocabulary, were what dictionary 0 was missing.** It is a raw dictionary — content with no literal, offset, match-length or literal-length tables at all. Its 130 bytes of `subject:`/`re:`/`wrote` are genuinely the right words and bought about 1%, which is the measurement that explains four versions of disappointment.
    - **`zstd --train` was not used and is not needed.** klauspost's `BuildDict` builds the entropy tables from samples and takes the dictionary content verbatim; it does not implement COVER or fastcover, so the substring selection is written out in `tools/traindict` — deterministic, greedy by estimated bytes saved, about eighty lines. Shelling out to the reference C binary would make a committed wire-format artifact depend on a tool nobody has installed, which §4's cgo-free reasoning applies to just as well.
    - **The corpus is synthetic, and that is a licensing decision.** A zstd dictionary embeds literal fragments of whatever it was trained on into the shipped binary. Training on archived message bases would put third-party text of uncertain provenance inside an MIT program, so `internal/dictcorpus` is written for the purpose and carries the repository's licence. It is a plausible imitation of BBS traffic rather than a sample of it, and the honest consequence is that a real archive with a compatible licence should beat it — §7.4 particularly wants FTN echomail, whose `SEEN-BY`, `PATH` and `MSGID` kludge lines are extremely repetitive. Train and holdout share a register and share no content; every figure above is from the holdout half.
    - **16 KiB of content, chosen by the generalisation gap rather than the ratio.** Holdout is still improving at 32 KiB, so the reason to stop is not that more stops helping: it is that a 33 KiB dictionary built from a 113 KiB corpus is carrying a third of its training data, and at 64 KiB the dictionary reaches 3.19× on data it has seen while gaining 0.026 on data it has not. Past that the holdout number stops being trustworthy in its own right, since this holdout shares a register with the training half by construction. What would justify a bigger dictionary is a bigger and more varied corpus, not a bigger budget over this one.
  - **Built in v0.23, and the second bullet is now done.** A node advertises the highest dictionary it holds in its digest (§7.3), records what every peer advertises, and compresses with the lowest level any peer it can currently hear has claimed. Four things this settled that the bullet did not say.
    - **"Highest held" is a complete description, not a lossy one.** A bitmap of which dictionaries a node has would spend seven bits on states this design forbids: old dictionaries stay supported and a set is built from 0 upward, so holding N means holding 0..N. A gap is unrepresentable because it is not a thing.
    - **There is no per-peer choice, because the mesh is broadcast.** `SendRecords` emits one bundle that every listening node hears, so a node picks a single dictionary and lives with it. The consequence is worth stating because it surprises people: **one peer on an old build holds the entire mesh at its level.** That is not a defect in the rule, it is what broadcast means — the alternative is encoding the same records once per peer, which spends the airtime of N transmissions to save the bytes of one and gives up the broadcast economy §7.2 chose fountain coding to get. So it is reported instead: the log names the peer and the remedy, once per change rather than every cycle.
    - **Two kinds of peer deliberately do not vote.** One that has never sent a digest has said nothing, and silence is not a claim — treating it as dictionary 0 would pin a fresh node at 0 permanently, since a node that has heard nobody would have nothing to raise it. One past `PeerTimeout` is gone by the same definition the digest interval already uses, so a mesh that loses its laggard recovers on the next cycle rather than at the next restart.
    - **Sneakernet declares rather than negotiates, permanently.** A stick has no round trip and may be handed to a board never heard from, so there is nothing to negotiate with — ever. The carrier envelope (v3) states the dictionary its bundles need, and an import refuses in one sentence before parsing anything: *this carrier needs dictionary 2, this build holds up to 1*. The per-bundle ID was already on the wire, but only reachable after committing to the parse, which is the difference the version bump buys.
    
    A node that hears something it cannot decompress now counts it separately (`rx_unknown_dictionary`) instead of in the same bucket as malformed frames, because the remedy is at the receiving end and everything that peer sends will fail identically until it is upgraded.
...
- **The mesh `Link` implementation rejects file payloads.** This is a type-level constraint, not a config option: `FILE_DATA` is not in the set of record types the mesh link will serialize. There is no sysop flag to turn it on and no size threshold below which it's allowed.
  - **Enforced one layer lower than this says, in implementation.** A link moves opaque datagrams — by the time bytes reach one, nothing knows they were a record — so a check there could only ever be a size heuristic, which is the thing this bullet forbids. The real hole is that the generic record path allows an 8 KiB body (`MaxBodyLen`), and 8 KiB is a small file. So a `FILE` record's body must parse as a catalog entry, checked both where a record is built and where one is decoded from a peer. Every field of an entry is bounded, so the largest possible one is 211 bytes against that 8 KiB ceiling: there is nowhere to put content and no threshold to relax. This also makes the rule link-independent, which is the right shape — a catalog entry stuffed with file bytes is wrong over IP too, and §7.5's fetch path 1 is a direct transfer between BBSes rather than a record.
- Fetch paths: (1) direct IP from a holding BBS, (2) queued for the next sneakernet exchange. That's it.
- Be honest in the UI: "held by `pnw` (`K7QM4X2P…`) — no IP route from here; queued for next exchange" is a feature, not a failure. (Both halves ship as written, as of Phase 5's request queue — see §6.5.)
- **The one asymmetry, stated so it is not read as an exception.** File bytes may ride a sneakernet carrier and may not ride a record. That is not this rule being relaxed for a friendlier medium: a blob on a stick is not in a record and not in a bundle, it is a separate section of a file on a drive, and the codec-level rule above is untouched. `[D8]` said "catalogs replicate; bytes move over IP or sneakernet" — a stick has no airtime and no §1.1 budget to spend, which is the whole of why the two differ.
...
- **Priority classes:** control (digests, delta requests, `NODE` and `SUCCESSION` records) > DM > forum posts > file catalog > door events. Under backpressure, drop from the bottom. There is no mesh-file class because there are no mesh files `[D8]`.
  - **The bottom rung was added in Phase 5** (§9.5). Door events go below the file catalog rather than beside forum posts where §9.5's caveat put them, because a stale catalog entry is still true and a stale battle report is noise. Implemented as reserves: 0.10 for DM, 0.35 forum, 0.60 file catalog, 0.70 door events, so classes fall out of contention in reverse priority order as the bucket drains.
- **Per-area sub-budgets** (§6.3) so one chatty area — or one FTN-bridged echo (§7.7) — cannot consume the node's entire allocation. **Built in Phase 5**, with a door league as its first user.
...
**What the survey samples is not what the telemetry interval configures, and the difference cost an afternoon.** §7.8.1's two figures are read from the node's own nodedb entry, because a self-addressed telemetry request is answered with a routing error rather than metrics. How often that entry is refreshed is a *different thing* from `device_update_interval`, which governs how often the node BROADCASTS telemetry to the mesh — and on a Heltec V3 on firmware 2.7.15 the two disagreed by a factor of sixty: the interval read 3600s while uptime advanced in exact 60-second steps and both utilization figures moved with it. The first version of `mesh survey` read the declared interval and refused on it, which blocked a measurement the radio was perfectly capable of producing, and sent a sysop chasing a setting that does not control the observable — through the CLI over TCP, over USB serial, through the web serial app and the phone app, none of which could make it take. **So preflight measures the refresh rate instead of reading it**, watching until it has two fresh readings and refusing on what it saw. The remedy when it does refuse is the telemetry *module* being switched off, not the interval being too long; that switch is what actually stops the refresh, and an earlier observation of this same radio sitting motionless for four minutes was taken with it off. Two smaller consequences: `mesh info` labels the field **mesh broadcast** rather than "metrics update", so the two lines do not invite the same inference; and a survey can now be **checked without being run**, since the answer costs minutes, the run costs hours, and until that existed the only way to learn a node could not be sampled was to commit to the whole thing.

**The two telemetry figures live on different timescales, and three separate calculations assumed otherwise.** This was measured rather than reasoned about, after the first hardware sweep returned R *falling* with hop limit — 9.5, 3.3, 1.8 across hops 1, 3 and 5, which is backwards, since more hops can only add rebroadcasts. A burst of ~19s of airtime drove `channel_utilization` to **17.46%** and it was back at ambient **90 seconds** later; over the same burst `air_util_tx` went 0.356 → 0.851 and was still **0.814 thirty minutes after the last packet**. So the channel figure is fast, of the order of the metric refresh itself, and the transmit figure spans about an hour. An hour-scale average could not have reached 17% from that much airtime at all, which is what makes the peak decisive on its own rather than resting on the decay curve.

Three consequences, all of them the same mistake in different places. **R's denominator is now our own duty cycle**, computed from what the survey sent rather than read from `air_util_tx`: the numerator is fast, so the denominator must describe the same window, and differencing an hour-long average against an opening baseline made each phase carry the ones before it — three identical 1.07% loads reported 0.308, 1.011 and 1.353, which is the entire inverted curve. **Drift is a plain subtraction again.** The elaborate residual correction §7.8's earlier text justified existed to remove a lag the channel figure does not have; it produced a range of −8.32 to +0.78 points against rises of 2 to 3, and both baselines are silent so both read pure ambient. **And the airtime governor's own abort guard** discounted our contribution using `air_util_tx`, so early in a phase it discounted nothing and the survey mistook its own load for somebody else's. Two smaller corrections fall out of the same measurement: the closing baseline discards its first two minutes, because the channel figure is still carrying the load phase that just ended, and a load phase stops transmitting two minutes before it stops sampling, because a packet sent at the boundary has most of its effect after it — which biased R *downward*, the unsafe direction, since under-stating R over-spends §1.1's budget.

**The simulator hid all of it, and that is the more useful lesson.** The fake mesh modelled both figures as one slow average. That is self-consistent, so `delta ÷ tx` recovered R exactly and the suite was green — a fake can be wrong in a way that is invisible precisely because it is coherent. It now derives the two figures on different timescales from the same transmission log, which is the only reason the fix could be verified at all.

**A node that refreshes slowly is not refused, it is repriced.** The gate is samples per phase — five, below which a mean and a standard error claim a precision the node never gave — rather than cadence against the sample interval. That leaves the sysop a second remedy the original check did not: lengthen the phases until a slow node has time to produce enough readings. Which also means the honest refusal names both causes, because a probe that saw nothing cannot distinguish a module that is off from one that is merely slower than the window, and asserting the first is the mistake this whole paragraph exists to record.
...
| **3. Client-held** | Only the user | needs a local helper | 5 — **implemented** |
...
- **Design now so tier 3 stays possible:** the server must never *require* the private key for anything except decrypting for display. Key discovery, DM addressing, signature verification, and delivery must all work from public keys alone. Getting this boundary wrong in Phase 1 is what would make tier 3 a rewrite rather than an addition.
  - **It was not, and tier 3 was an addition.** The whole server-side mechanism is one row: a `dm_public_key` with `dm_wrapped_key` NULL **is** a tier-3 user. Sending, discovery, verification and delivery needed no changes at all, because none of them ever reached for the private half. The instruction above is the reason, and it is worth recording that a constraint adopted in Phase 1 and honoured for four phases paid exactly what it was supposed to.

#### 8.2.1 Tier 3 as built

`meshbbs-key` is a **separate binary**, not a subcommand. A subcommand would live in the same executable a sysop runs on the machine holding everyone's mail, which is the association tier 3 exists to break. It ships for every target the server does, and the reason it ships for more is that the platform it runs on is the *user's*.

Three commands — `init`, `pubkey`, `open` — and no new cryptography: `keyring`'s `Wrap`, `Unwrap`, `Seal` and `Open` unchanged, per `[N3]`'s insistence that this is the wrong place for cleverness.

**Ciphertext crosses the gap as a pasted block**, delimited and base64 at 64 columns. Three properties are load-bearing rather than cosmetic. The header names the contents, because a bare blob is indistinguishable from every other bare blob — including a wrapped *private* key, which is the one thing that must never be handed to anybody, and "paste your message here" has to be a safe instruction. Everything outside the delimiters is ignored, because a user copying from an 80-column terminal takes the prompt and status line with them and refusing that paste teaches them to trim by hand, which damages the block. And two blocks are refused rather than resolved to the first: a user who pastes two messages and gets one plaintext cannot tell which one they are reading.

**The passphrase does not come from stdin**, which is the one place this departs from the repo's `--password-stdin` convention. `open` reads the *ciphertext* from stdin, so the two would be the same stream. It is read from the controlling terminal instead — `/dev/tty`, or `CONIN$` on Windows — which keeps it unechoed and out of shell history; `--passphrase-file` serves scripts and honours the reason the convention exists, which is that a secret must never reach argv.

**Four refusals, all about losing mail rather than being wrong.** `init` will not overwrite a key file, and checks *before* asking for a passphrase, because prompting and then refusing invites a user to type it somewhere it was never needed — overwriting destroys every message ever sent to the old key, with no error at the time, since the mail is still in the inbox and is now ciphertext nobody can open. Adopting a *different* key on the server strands mail already delivered for the same reason and needs `--replace`. `open` reads the message before asking for the passphrase, so a user pasting into a pipe silently waiting on a hidden prompt does not type their passphrase into the paste. And a world-readable key file warns rather than refusing: it is ciphertext, so the passphrase still stands between an attacker and the mail, and refusing to run would punish a permission bit while somebody's mail sat unread.

**What tier 3 does not fix.** `user dm-key` is a sysop command and that is a weak control, stated as weak: a user able to set their own key over SSH could set one for an account whose password had leaked, redirecting every future message and locking the owner out with no notification anywhere. It is a speed bump on the one operation whose failure is silent, in a trust model that is already sysop-to-sysop `[D5]`. Nothing here authenticates a public key to a person — that is what §8.2's trust-on-first-use fingerprint comparison is for, and it is unchanged.

**And what it costs the user.** Reading mail stops being something the BBS does. Every message is a copy, a paste and a passphrase, on a machine that is not the one they are logged into. Tier 2 remains the default for that reason; tier 3 is for people who want it and will pay for it.
...
**Implemented in Phase 4, and specified in [doors.md](doors.md).** That document is the contract a third-party door author writes against; what follows here is the decision record it was built from. Four things it narrows are worth knowing without opening it: a door's announce area **must** be local rather than merely can be (§9.1.1 below, and §11.4's asymmetry about shared airtime); the rendered author `TRADEWARS (door@K7QM4X2P…)` does not fit a post's 16-byte author field, so a marker is stored and the rendering is computed; dropfiles are written **per invocation** rather than per node; and **memory limits cannot be enforced on macOS at all**, so a door configured with one refuses to launch there rather than running unbounded.
...
**Implemented in Phase 5.** The door-facing contract is [doors.md](doors.md) §6.7; what follows is the shape it took, and the arithmetic that left no choice about it.

**A league *is* an area.** Not a `leagues` table: an area of kind `door`, sharing the `areas` table with forums (§6.3) and file areas (§6.5) exactly as those two share it. A league needs a named namespace that some instances federate and others do not, with its own sequence space, its own retention and a version vector the engine reconciles — which is the whole of what an area is. A parallel table would mean reimplementing anti-entropy, digests, bundles, the fountain codec and the sysop CLI for a feature whose entire appeal is that §7 already carries it. It also inherits the constraint that put file areas here: an `AreaTag` is derived from the name in one global namespace, so independently named leagues would let a forum and a league silently merge into one tag. Two refusals keep the kinds apart, both applied where a record is accepted rather than where it is read: a `POST` arriving for a league is refused, and a `DOOR_EVENT` arriving for a forum is refused. Refusing rather than storing is the part that matters — a record this node accepts is one it will **relay**, so a single confused peer would otherwise spend everybody else's airtime. The per-area sequence space of §6.3 matters more for this kind than for either other one, because a door game is the chattiest thing this document contemplates.

**Batching happens inside the record, and the arithmetic forces it.** A record's framing is **89 bytes, of which 64 is the Ed25519 signature**, and a typical one-event body — a LORD kill — is **23 bytes**, making that record **112 bytes** and spending 79% of the wire on framing. "Small and batched" therefore cannot mean batching records into bundles: a bundle already hoists origin, area and base timestamp (§6.1.3), but every record inside it keeps its own signature. The only batching that pays is inside the body, and it pays a great deal — computed from the codec by §12.7, not estimated:

| Six events | Bytes |
|---|---:|
| one per record | 672 |
| in a single record | 197 |

**3.4×.** Everything else follows from that table. One record is about one game, so the game name is hoisted onto the record instead of repeated per event. **Eight events per record** is the cap: it captures nearly all of the 3.4× while keeping the worst legal body at **882 bytes**, under the explicit `MaxDoorEventBodyLen` of **1024** and far under §6.2's 8 KiB `MaxBodyLen`. That explicit second ceiling is not redundant — without it the generic 8 KiB body would make every per-field bound a suggestion rather than a rule, which is the identical hole §7.5 closes for `FILE`.

The fields are bounded so the argument above stays true: a game name of 16 bytes (a key, matched exactly, so every board in a league must type it identically), an actor and optional target of 24 bytes (`MaxNickLen`, reused rather than given a second bound for the same thing), an 8-byte target node written only when there is a target, and 48 opaque door-defined payload bytes. **`kind` is a `uint8`**, not a string: MeshBBS cannot enumerate any door's events, a name would be paid on every entry, and 256 codes per game is more than any door will use. An unknown code is forward-compatible by construction — a receiver renders "event 7" and passes the payload on. The 48-byte payload bound is what keeps §7.5 true by arithmetic rather than by inspection, and is therefore not a tuning knob: a door wanting more than that is doing state synchronisation, which this is not.

A partial batch waits out a window **derived** from what the area's share actually buys rather than a configured default, because the right window depends on R and R is unmeasured (§14, `N10`). A configured default would have to be chosen at one R and rewritten at another, leaving every sysop who had already set one holding the wrong value.

**`DOOR_EVENT` records are node-signed**, consistent with `[D5]`. The question was whether crossing a trust boundary and driving game state on someone else's machine warrants something stronger — and it does not, for a reason worth writing down: the threat in an inter-BBS league is a **cheating sysop**, not a cheating user, and user-signing provides zero additional protection against that. Under tier-2 key custody (§8.2) the sysop's server holds the user's key material during a session and can produce a valid user signature at will. Node-signing is therefore both cheaper and *more honest about who is actually being trusted* — leagues rest on sysop-to-sysop trust, exactly as they did in the FidoNet era. A league that wants integrity guarantees against its own members needs game-level design (commit-reveal, cross-checked state), not a signature layer. The same limit applies one level down: a door may name a target on another board, and that is a **claim by the origin node**, not a verified fact.

**Corrected in implementation: door events sit BELOW the file catalog, not alongside forum posts.** This section's own caveat, citing §1.1's budget, said `DOOR_EVENT` shares the bottom of the priority order with forum posts; §7.6's ladder stopped at the file catalog and named no rung for it at all. Both are now refined rather than contradicted — `ClassDoorEvent` is a new bottom rung at reserve 0.70, below `ClassFileCatalog`'s 0.60 — and the reason is the whole content of the decision. **A catalog entry that arrives a week late is still true**: the file is still there, the hash still matches, and somebody searching next month finds it. **A battle report that arrives a week late is noise** — the game has moved on, nobody is waiting for it, and it is competing with a person's post for the same second of channel time. Between a person's words and a game's telemetry, the person wins. It also makes the feature safe to ship before `N10` is measured: a league that turns out to be too chatty for a real mesh degrades by losing its own traffic first rather than by pushing somebody's mail off the air.

The caveat that survives is the other half of §1.1's: **a chatty game will notice**, and the sub-budget it asked for is §6.3's per-area `airtime_share` — specified in Phase 1, built in Phase 5, with the league as its first user rather than its reason. A class reserve is not a substitute: a reserve stops a class spending the *last* x% of the bucket and says nothing about rate, so a chatty area inside a 0.70 reserve can still drain 30% of the day's budget in an hour.
...
`airtime_share` is **built as of Phase 5** (§6.3) rather than listed here and nowhere else: a fraction of this node's budget, 0 meaning uncapped, set with `meshbbs area share`, and the shares of federated areas are refused if they would sum past 1
...
- Door leagues — Phase 5 (§9.5): `door_event_batch_window` (default `auto`, which derives the window from what the area's share buys so that a measured R propagates without a config edit; an explicit duration is honoured and clamped to 5m–12h, and says so rather than being silently ignored), `door_event_max_age` (default 24h, after which a queued event is dropped unsent and logged — it exists to stop a node that has been offline for a week spending its whole budget on a game that finished, not to tighten latency). Both are validated by `config check` rather than at first use
- Safety: `ham_mode_override` (`i_accept_part97_responsibility`) `[D11]`

**Doors — Phase 4**
Per-door (database): `name`, `path`, `args`, `cwd`, `env_passthrough`, `dropfile_type`, `max_concurrent`, `node_lock`, `cpu_limit`, `mem_limit`, `wall_clock_limit`, `required_capability` (what a *user* needs to run it — and, since a league grant is per door rather than per user, the knob for a sysop who wants a user-level gate on §9.5 too), `api_level` (1–3 by default; `act_as_user` is level 4 and must be set explicitly, §9.1.1), `announce_area`, `announce_rate_limit`, and — Phase 5 — `league_area` and `league_per_hour` (§9.5): the **federated** door league this door reports to and reads back, and how often it may report. Empty means it may not emit, which is a different thing from a rate limit of zero. Not a fifth API level: levels nest, and a fifth above `act_as_user` would claim that reporting a game result is more authority than posting as the user
...
**Built in v0.21**, in `internal/conformance/testdata/v1` with a generator in `tools/conformance` and a named CI step on all three platforms. Five things it settled that these four bullets did not say.

**The gap was not "are the encoders tested" but "tested against what".** Every existing test that touches an encoder is a round trip, and a round trip proves the encoder and the decoder agree with each other — not which bytes they chose. This was measured rather than asserted: flipping the record timestamp to little-endian in `canonicalBytes` *and* `decodeCanonical` together leaves the entire test suite green, `internal/record`'s own tests included, and fails only here. Flipping just one side is caught everywhere, which is why the interesting break is the coordinated one.

**Scope is the whole stack, not just records.** The first bullet names records, but the fourth is what decides the size: a third-party implementation needs identity derivation, the record envelope, the typed bodies, bundle framing, the gossip control plane, version vectors, the fountain symbol header, and the signed `ANNOUNCE`. Ninety-one vectors across seven files, cross-referenced by name so a bundle vector is a statement about composition rather than a second copy of the record vectors.

**The mask matters more than the envelope, and that is not where anyone would look first.** A repair symbol's mask is derived from `(sender, bundle_id, index)` and never transmitted (§7.2). An implementation that derives it differently fails no length check and no signature check — it decodes to garbage, silently. Every other break in this format announces itself; this one does not, which makes it the vector a third party most needs and the one nothing outside `internal/fountain`'s own round-trip tests pinned.

**Bundles pin the uncompressed body.** `Pack` compresses, and compressed bytes move with the zstd library and will move again when §7.4's trained dictionary lands — pinning them would freeze a compressor's behaviour and call it a wire format. `bundle.EncodeBody` is the format. The compressed direction is a frozen blob asserted only to *decode*, which also gives §7.4's "old dictionaries stay supported" its first enforcement: retraining that edited dictionary 0 in place rather than shipping a dictionary 1 fails here and nowhere else.

**Immutability had to be mechanical.** "Generated once and thereafter immutable" survives exactly until the first red build, where regenerating is the obvious fix and is the one that converts a wire-format break into a green test and a diff nobody reads. So the generator recomputes every existing vector, refuses to write if one would change, and refuses to let one be deleted — a third party may already be testing against it. The refusal is the one `identity.SaveNodeKey` makes about a node key, for the same reason.

One limit stated rather than implied: **a corpus freezes current behaviour including its bugs**, which is an argument for doing this before the freeze rather than as part of it — after the freeze a bug in the encoding stops being a bug and becomes a compatibility problem. The vectors were generated only after all eight wire-format fuzz targets ran clean, since three canonicalisation bugs in this codebase were found by fuzzing and none by reading.

**The third bullet was closed in v0.23, by the first format change that came along.** §7.4's dictionary byte took gossip to wire version 2, which collided head-on with the append-only corpus — the frozen `digest-*` vectors assert version 1 bytes and may not be regenerated. That collision is the mechanism working, not a problem with it, and the resolution is what the bullet asked for.

The corpus is now **generations**: `testdata/v1`, `testdata/v2`, each a snapshot of all three wire versions as they stood when it was frozen. A snapshot of all three rather than a directory per format, because the only combination worth testing against is one that actually shipped together. An old generation is never deleted and is not dead weight — it becomes the cross-version corpus and tests in both directions at once. Layers whose version did **not** change must still encode byte-identically, which is the half nobody thinks to check: when one format moves, the risk is to the four that did not, because the current generation is regenerated at that same moment and would happily freeze a fresh mistake. Layers whose version **did** change must now be **rejected**, with a version error rather than a truncation, which is §7.1's pre-freeze drop-and-log written as an assertion.

**It found a real bug on its first run, and the bug is instructive.** `DecodeDigest` checked the version-2 header length *before* the version byte, so a three-byte version-1 digest came back as "truncated gossip message" — which sends a sysop to look at the radios when the answer is that a peer needs upgrading. The general rule fell out of it and now holds in every gossip decoder: **a length rule belongs to the version that defined it**, so the version is read before any structural check. Nothing but a corpus of genuinely old bytes would have caught this; a round-trip test cannot, because it never has any.
...
**The byte budget was asserted from Phase 0; the compression gate arrived in v0.22 with the dictionary it exists to watch.** `internal/bundle/compression_test.go` fixes per-shape floors and an overall floor against `internal/dictcorpus`'s holdout half, plus two assertions worth naming because they are not ratios. Dictionary 1 must beat *both* plain zstd and dictionary 0 on every shape — a dictionary can improve the aggregate while making one record type worse, which is the exact failure the bullet above describes. And **no bundle may come out larger than it went in**: plain zstd expands a lone catalog entry, and a compressor that adds bytes on a link where §1.1 counts them is worse than none. Two structural points, learned the hard way: the gate measures the **holdout** half, since on the training half the same dictionary reads 3.19× against a real 1.41× at a 64 KiB budget; and it uses the same batch-size mix the trainer samples, because an "overall ratio" that differs from the trainer's only by weighting is two numbers for one fact and will eventually be quoted wrongly. The other two gates in this list remain unbuilt, and there are no benchmarks in the tree at all.
...
| **4** | Windows/ConPTY runner; door sandbox escape attempts; dropfile fuzzing. **Delivered**, with one change of shape: the "escape attempts" attack the boundaries that exist — a stolen token, a door reaching another door's state, an announce flood, a request flood, an operation spelled to slip past its level — and deliberately do not attack the ones that do not, since a green test for an undefended boundary implies a defence |
| **5** | A fuzz target for the `DOOR_EVENT` body (§9.5), and the league crossing exercised in simulation. **Delivered for `DOOR_EVENT`.** The fuzzer found nothing in 43 million executions, which is a claim about the test as much as the code, so it was checked: with the trailing-byte rejection and the canonical re-encode defence removed it produced a second wire form in 0.14 seconds. The crossing runs end to end in the simulator: two instances with real stores and real gossip over a lossy simulated mesh, an event reported through the door host on one board and read back through the door host on the other, plus a second test asserting that six results from two boards arrive as two records rather than six |
| **6** | FTN parser fuzzing and loop-prevention tests on a cyclic topology; cross-version conformance |
...
| **4 — Doors** | Modern door API + spec **incl. the §9.1.1 capability model**, PTY/ConPTY bridging, sandboxing, 2–3 reference doors, dropfile generation, **Windows CI runner (§12.9)** | Independent of federation; parallelizable with 2–3. Now the whole of door scope. **Code-complete — see below and [doors.md](doors.md).** |
| **5 — Reach** | ~~Web terminal~~ (**shipped early — see below**), inter-BBS `DOOR_EVENT` (**code-complete — see below**), sneakernet bundles, **`meshbbs-key` helper for client-held DM keys (tier 3)** | Genuinely optional, except tier-3 keys which are a stated preference |
...
**Phase 6 has begun at its prerequisites rather than its deliverables.** §12.6's conformance vectors are built (v0.21), §7.4's dictionary is trained and shipped (v0.22), and its negotiation and the first cross-version corpus landed together (v0.23) — none of it because the freeze is close, but because all of it is worth more before the freeze than during it. A corpus freezes current behaviour including its bugs, and a bug caught after the freeze is a compatibility problem rather than a fix; a dictionary and a protocol version are cheap changes today and interop commitments afterwards.

**Everything that can be done without radios is now done.** The two prerequisites left both need hardware and somebody's afternoon, which is the honest summary of where this project stands: the wire format is as ready as reading and simulating can make it, and the remaining questions are ones only the air can answer.

**~~Dictionary negotiation~~ — done in v0.23.** Nodes advertise the highest dictionary they hold in their digest, and compress with the lowest level any peer they can hear has claimed (§7.4). The debt v0.22 took on is paid: a dictionary 2 can now ship after the freeze, because there is a way to find out who can read it. It also cut the first cross-version corpus, since the digest change was the first format bump the append-only vectors had ever faced (§12.6).

**`N10`.** The governor runs on a guessed flood multiplier of 4, every airtime figure in this document scales linearly with it, and the framing about to be frozen was sized against that guess. `mesh survey` exists to measure it; nobody has.

**Real radios.** §13's caution about not rushing toward Phase 6 is really about this. The three deliverables — the written spec, the portnum request, the FTN gateway — are all promises to strangers about a format, and this one has never carried a full record sync between two instances over the air, never crossed a league between two boards, and never been walked between two machines on a stick. Groundwork is not what should wait; that is.

**Phase 3 is code-complete as of v0.17.** All five items in its row are implemented: serial and TCP transports with protobuf framing, the airtime governor with flood-multiplier accounting, ham-mode safety checks, `mesh survey`, and file catalog replication. Two things are worth stating precisely rather than letting "complete" imply them. What hardware has shown is the LINK — §7.1.2.1's two-radio bring-up established discovery, broadcast and attribution; a full record sync between two instances over the air is not yet recorded here. And `N10` is still open: the governor runs on a guessed flood multiplier of 4, every airtime figure in this document scales linearly with it, and §14 wanted it measured before the governor's defaults were finalized. `mesh survey` now exists to measure it, which is the part that was blocking.

**Phase 4 is code-complete.** Every item in its row is implemented: the door API with all four §9.1.1 levels, PTY and ConPTY bridging, the limits §9.4 asks for, two reference doors, all three dropfile formats, and a written spec in [doors.md](doors.md). It also closed a Phase 1 omission — `users.session_time_limit` had been in §11.5 since the design was written and never built — because §9.1 promises a door the time remaining in the session and there was no session clock to read.

Four things are worth stating precisely rather than letting "complete" imply them.

**Memory limits cannot be enforced on macOS.** Setting `RLIMIT_AS` or `RLIMIT_DATA` returns `EINVAL` there, and a child asked for 256 MiB against a 64 MiB limit gets it — measured, not inferred. A door configured with a memory limit refuses to launch on darwin rather than running unbounded, because §9.4's list does not contemplate a platform that cannot deliver one and silently ignoring it is the one option that must not exist. Linux and Windows enforce it.

**A door written in Go ignores the CPU limit.** The Go runtime delivers `SIGXCPU` to a listener and ignores it when there is none: a Go child spun ten seconds against a one-second limit while `awk` died in one. This is survivable only because the wall-clock limit is required and applies to every door whatever it is written in — which is why that one is not configurable away.

**§9.1's "sandboxing" is not implemented, and was never going to be.** §9.4's actual bar is a dedicated low-privilege account, documented rather than enforced, and that remains the thing limiting a door's reach. What Phase 4 does enforce is that a door cannot run forever and cannot leave anything behind. The adversarial suite attacks the boundaries that exist and deliberately does not test the ones that do not.

**Everything Windows-specific is unverified by a human.** ConPTY, the job object and the named pipe compile on every change and execute only on the CI runner §12.9 calls non-negotiable. `meshbbs dev door-run` exists so a sysop can try a door by hand there, because CI can prove the code paths run and cannot say whether the screen looks right.

**Phase 5's `DOOR_EVENT` is code-complete.** The wire format (§9.5), the league area kind and its two refusals, the priority class and its reserve, §6.3's per-area sub-budget, `event.emit` and `event.poll` ([doors.md](doors.md) §6.7), the batching flusher, the arrival-order cursor, and `meshbbs door events` for the sysop who pays to carry a league.

**Phase 5 is code-complete.** Sneakernet bundles landed as `meshbbs sneakernet export/import` — a container of the existing bundle format plus the sender's version vectors, so two boards that have never spoken converge across two hand-offs — and with them §6.5's fetch path 2, the request queue that turns "held by another BBS" from a dead end into a wait. Every item in the row is now built.

Three things are worth stating precisely rather than letting "complete" imply them.

**No two real boards have exchanged a stick.** What has been exercised is the whole path through the built binary, twice over, ending in identical vectors and a requested file downloadable at the far end — which is what §7.3 means by converged. Nobody has walked a USB drive between two machines, and until somebody does, the parts of this that are known to work are the ones a test can reach.

**A carrier is unsigned and does not need to be.** Every record inside carries its own signature and goes through the same `GossipStore.Apply` a radio's records do, so a forged carrier cannot introduce a record its claimed origin did not write. File bodies are content-addressed and re-hashed on the way in, which is what makes an unsigned container safe to take bytes from. The origin and vector fields are hints about what to send back; the worst a liar achieves is that somebody wastes a trip. This is a **worse** threat model than a peer's, not a better one — an attacker chooses the bytes and the size, and a sysop plugs it in on purpose — so every count and length is bounded before it is allocated against.

**A carried file nobody asked for is content without a catalog entry.** A body arrives with a hash and a length; the name lives in a `FILE` record, which may or may not be on the same stick, and there is no area to file it in and no name to file it under. A request is what supplies both, which is the sharpest argument for the queue existing at all. A blunt `--files` export still works and the import says plainly how many bodies landed with nothing to reach them by.

Five things are worth stating precisely rather than letting "complete" imply them.

**No inter-BBS league has ever run between two real boards.** What has been exercised is the local path end to end — a door emits, the event is queued, batched, signed and published onto a federated area — and a crossing put together in simulation from records signed by a second node's key. Nobody has played a game across two instances, over a radio or otherwise, and until somebody does, the parts of this that are known to work are the ones a test can reach.

**The batch window is derived rather than chosen, and that is a consequence of `N10`, not a flourish.** The right window depends on R. A configured default would have to be picked at one R and rewritten at another, and every sysop who had already set one would keep the wrong value — so `auto` computes it from the governor's own arithmetic instead: an area whose share buys P full packets a day waits a day divided by P, clamped to 5m–12h. Halving the budget doubles the window with no other change anywhere. The honest reading is that nobody knows what the right window is, and this is the arrangement in which finding out costs no code and no config edit.

**A league carrying more than a handful of events a day does not fit the budget at any plausible R.** At fifty instances and the guessed R of 4, a node originates about 11 full packets a day; a league area held to a tenth of that share buys about one, which is one record, which is at most eight events. Uncapped, a league could take the node's whole allocation and leave nothing for mail or posts. Both figures scale inversely with R and R is unmeasured — at R = 8 they halve. So an inter-BBS league here is a scoreboard that settles over days, not a live feed, and §9.5's "small and batched" is a harder constraint than it sounds. The bottom-rung priority class is what makes shipping this safe before the measurement: a league that is too chatty for a real mesh loses its own traffic first.

**`truncated` is plumbed for a pruner that does not exist.** Retention is a per-area policy (§6.3) and nothing prunes records yet, so the flag a door must branch on to avoid showing an incomplete league table has never fired in production. It was built now rather than with retention precisely because that is when it would have been forgotten — and migration 0010's arrival counter exists for the same reason, since SQLite reuses `max(rowid)+1` after a delete and the resulting bug is a door silently missing events months from now, on a board that has been running long enough for retention to have started.

**There is no built-in league listing, deliberately.** A scoreboard is what a door renders, and only the door knows what kind 3 means. The surface that was actually missing is the sysop's: a league is the one kind of area with nothing that would otherwise mention it — a forum's posts are in the forum and a file area's catalog is browsable, while a league's records are interpreted by a door that may not be installed. Records for a game no local door plays are **kept**, because a door installed tomorrow should be able to catch up and because a node carrying a league it does not play is how a multi-hop league reaches boards that cannot hear each other, which is the FTN pass-through pattern inter-BBS leagues came from. That costs this node airtime to serve, so `meshbbs door events` says so, once, with the remedy.
